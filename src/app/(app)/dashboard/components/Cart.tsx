...
function calDiscount(discount: any) {
  const totalDiscount = Object.values(discount).reduce(
...
const Cart: React.FC<Props> = ({
  ...
  const [tax, setTax] = useState<any>(0.0);
  ...
  useEffect(() => {
    if (itemInfo.price !== undefined) {
      ...
      const newObject = { ...quantity };
      newObject[itemInfo.barcode] = 1.0;
      setQuantity(newObject);
      const newDiscount = { ...discount };
  ...
  return (
    <div className="w-1/4">
      <Card
        className="overflow-hidden w-full h-[730px]"
      ...
      >
        <CardHeader className="flex flex-row items-start bg-muted/50">
          <div className="grid gap-0.5">
            <CardTitle className="group flex items-center gap-2 text-lg">
              <span>
                Order# <span className="font-extralight">{unique}</span>
              </span>
              <Button
                size="icon"
                variant="outline"
          ...
          <div className="grid gap-3">
                      ...
                      if (t.quantity != 0) {
                        return (
                          <li
                            ...
                            key={t.barcode}
                          >
                            <span className="text-muted-foreground w-36 text-wrap">
                              {t != null ? t.name : " "}
                            </span>
                            <div className="absolute left-40">
                              <Button
                                variant="outline"
                                className="h-6 w-6 p-0 m-0 ml-4"
                                data-custom={t.barcode}
                              ...
                              <span className="absolute left-9 top-1 ml-4">
                                {quantity[t.barcode]}
                              </span>
                              <Button
                                variant="outline"
                                className="h-6 w-6 p-0 m-0 ml-8"
                            ...
                            <span className="ml-30">{`$${
                              t != null
                                ? (quantity[t.barcode] * t.price).toFixed(2)
                                : " "
                            }`}</span>
                          </li>
                        );
            ...
            <ul className="grid gap-3">
              ...
              <li className="flex items-center justify-between font-semibold">
                ...
                <span id="total">
                  ${subTotal(items, quantity) - calDiscount(discount)}
                </span>
              </li>
            </ul>
          </div>
        ...
        <CardFooter className="flex flex-row items-center border-t bg-muted/50 px-6 py-3">
          <div className="text-xs text-muted-foreground">
            Updated <time dateTime={getDate()}>{getDate()}</time>
          </div>
        </CardFooter>
      </Card>
    </div>
